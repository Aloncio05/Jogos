'use strict';
// ── Corrida do Aloncinho v6 — Redesign completo ──────────────────────────────
...
const HOR_Y   = 200;
const BASE_Y  = H - 80;
const LANE_X  = [80, 180, 280];
const SPAWN_Y = BASE_Y - 200;
...
// ── POOL DE OBSTÁCULOS ─────────────────────────────────────────────────────────
function mkTrain(ci){const[a,b]=TRAIN_COLORS[ci];return{kind:'train',w:80,h:120,layer:'ground',span:1,ca:a,cb:b};}
const OBJ_POOL=[
  mkTrain(0),mkTrain(1),mkTrain(2),mkTrain(3),
  {kind:'barrier', w:280,h:28, layer:'ground',span:3,ca:'#ff6f00',cb:'#e65100'},
  {kind:'sign',    w:80, h:36, layer:'air',   span:1,ca:'#00bcd4',cb:'#006064'},
  {kind:'overhead',w:280,h:20, layer:'air',   span:3,ca:'#5c6bc0',cb:'#283593'},
];
...
const STARS=Array.from({length:55},()=>({
  x:Math.random()*W,y:Math.random()*HOR_Y*.9,
  r:.6+Math.random()*1.5,phase:Math.random()*Math.PI*2
}));
const BUILDINGS=[
  {x:0,  w:28,h:90},{x:30, w:18,h:65},{x:50, w:32,h:110},{x:85, w:22,h:75},
  {x:109,w:16,h:52},{x:127,w:28,h:95},
  {x:215,w:22,h:80},{x:239,w:18,h:58},{x:259,w:30,h:100},{x:291,w:20,h:72},
  {x:313,w:28,h:88},{x:343,w:17,h:60},
];
const CLOUDS=[{x:60,y:55,w:70,h:22},{x:200,y:40,w:90,h:28},{x:300,y:70,w:60,h:18}];
...
function spawnObstacle(){
  ...
  const x=def.span>=3?W/2:LANE_X[lane];
  obstacles.push({...def,lane:def.span>=3?-1:lane,x,y:SPAWN_Y-def.h});
}
function spawnCard(){
  const ck=CARD_KEYS[Math.floor(Math.random()*CARD_KEYS.length)];
  const lane=Math.floor(Math.random()*3);
  cards.push({ck,x:LANE_X[lane],y:SPAWN_Y,w:22,h:22});
}
...
function kills(o){
  ...
  if(o.layer==='air'&&P.jumpH>=100)return false;
  const pY=P.y-P.jumpH;
  const dy=Math.abs(pY-o.y);
  const dh=(P.crouching?P.h*.35:P.h*.48)/2+o.h*.32;
  return dy<dh;
}
...
function collectCard(c){
  ...
  popups.push({x:P.x,y:P.y-P.jumpH-40,text:`+${d.bonus}`,col:d.col,life:1});
}
...
// ── FUNDO ──────────────────────────────────────────────────────────────────────
function drawBg(){
  const sky=ctx.createLinearGradient(0,0,0,HOR_Y+40);
  sky.addColorStop(0,'#1a0533');sky.addColorStop(.3,'#6b1fa0');
  sky.addColorStop(.65,'#d4508a');sky.addColorStop(1,'#ff9f4a');
  ctx.fillStyle=sky;ctx.fillRect(0,0,W,HOR_Y+40);

  STARS.forEach(s=>{
    const a=.3+.7*Math.abs(Math.sin(frame*.04+s.phase));
  ...
  BUILDINGS.forEach(b=>{
    ...
    for(let wy=top+6;wy<HOR_Y-4;wy+=12){
...
function drawTrack(){
  ...
  const tileOff=(frame*spd*1.2)%60;

  const g=ctx.createLinearGradient(0,HOR_Y,0,H);
  g.addColorStop(0,'#160e28');g.addColorStop(.3,'#1a1230');g.addColorStop(1,'#0d0a1a');
  ctx.fillStyle=g;ctx.fillRect(0,HOR_Y,W,H-HOR_Y);

  ctx.fillStyle='#12082a';
  ctx.fillRect(0,HOR_Y,LANE_X[0]-42,H-HOR_Y);
  ctx.fillRect(LANE_X[2]+42,HOR_Y,W-(LANE_X[2]+42),H-HOR_Y);

  for(let i=0;i<=22;i++){
    const yy=HOR_Y+i*60-tileOff%60;
    if(yy<HOR_Y||yy>H)continue;
    const t=(yy-HOR_Y)/(H-HOR_Y);
    const x0=W/2-(W/2-(LANE_X[0]-42))*t;
    const x1=W/2+(LANE_X[2]+42-W/2)*t;
    ctx.strokeStyle=`rgba(124,111,205,${.04+t*.1})`;ctx.lineWidth=.8+t*1.8;
    ctx.beginPath();ctx.moveTo(x0,yy);ctx.lineTo(x1,yy);ctx.stroke();
  }

  const dashOff=(frame*spd*.9)%50;
  ctx.setLineDash([18,14]);ctx.lineWidth=2;
  [LANE_X[0]+40,LANE_X[1]+40].forEach(lx=>{
    ctx.lineDashOffset=-dashOff;ctx.strokeStyle='rgba(180,150,255,.2)';
    ctx.beginPath();ctx.moveTo(W/2,HOR_Y);ctx.lineTo(lx,H);ctx.stroke();
  });
  ctx.setLineDash([]);

  [LANE_X[0]-42,LANE_X[0]+40,LANE_X[1]-1,LANE_X[1]+1,LANE_X[2]-40,LANE_X[2]+42].forEach(bx=>{
    const rg=ctx.createLinearGradient(0,HOR_Y,0,H);
    rg.addColorStop(0,'rgba(160,130,255,0)');
    rg.addColorStop(.35,'rgba(190,165,255,.55)');
    rg.addColorStop(1,'rgba(220,200,255,.95)');
    ctx.strokeStyle=rg;ctx.lineWidth=2.8;
    ctx.beginPath();ctx.moveTo(W/2,HOR_Y);ctx.lineTo(bx,H);ctx.stroke();
    ctx.strokeStyle='rgba(255,255,255,.18)';ctx.lineWidth=.8;
    ctx.beginPath();ctx.moveTo(W/2,HOR_Y);ctx.lineTo(bx+1.5,H);ctx.stroke();
  });

  const floorGlow=ctx.createLinearGradient(LANE_X[0]-42,0,LANE_X[2]+42,0);
  floorGlow.addColorStop(0,'rgba(100,70,200,0)');
  floorGlow.addColorStop(.5,'rgba(130,100,255,.14)');
  floorGlow.addColorStop(1,'rgba(100,70,200,0)');
  ctx.fillStyle=floorGlow;ctx.fillRect(LANE_X[0]-42,H-35,LANE_X[2]-LANE_X[0]+84,35);

  if(P.turbo>0){
    ctx.save();ctx.globalAlpha=.18;ctx.strokeStyle='#00E5FF';ctx.lineWidth=2;
    for(let i=0;i<6;i++){
      const sx=15+i*55,sy=((i*80+frame*(baseSpeed+2)*3)%(H+80))-40;
      ctx.beginPath();ctx.moveTo(sx,sy);ctx.lineTo(sx,sy+60);ctx.stroke();
      ctx.beginPath();ctx.moveTo(W-sx,sy);ctx.lineTo(W-sx,sy+60);ctx.stroke();
    }
    ctx.restore();
  }
}
...
function drawObstacle(o){
  const range=BASE_Y-(SPAWN_Y-o.h);
  const t=Math.max(0,Math.min(1,(o.y-(SPAWN_Y-o.h))/range));
  const sc=0.62+0.38*t;
  const isClose=o.y>BASE_Y-85&&frame%8<4;

  ctx.save();ctx.translate(o.x,o.y);ctx.scale(sc,sc);
  if(isClose){ctx.shadowColor='#FF1744';ctx.shadowBlur=30/sc;}
...
function drawTrain(w,h,ca,cb){
  ctx.fillStyle='rgba(0,0,0,.38)';ctx.fillRect(-w/2+8,8-h,w,h);
  ...
  ctx.save();ctx.clip();ctx.fillStyle='rgba(255,255,255,.1)';ctx.fillRect(-w/2,-h,w,h*.22);ctx.restore();
  ...
  ctx.font=`bold ${Math.round(h*.1)}px sans-serif`;ctx.textAlign='center';ctx.textBaseline='middle';
  ctx.fillText('11',0,-h+h*.18);
  ...
  [[-w/2+14,-h+h*.68],[w/2-14,-h+h*.68]].forEach(([hx,hy])=>{
    ...
    hg.addColorStop(0,'rgba(255,250,180,1)');hg.addColorStop(.45,'rgba(255,215,0,.32)');hg.addColorStop(1,'rgba(0,0,0,0)');
...
function drawCard(c){
  const d=CARDS[c.ck];
  const bob=Math.sin(frame*.14+c.x*.05)*4;
  ctx.save();ctx.translate(0,bob);
  ctx.shadowColor=d.col;ctx.shadowBlur=18;
  ctx.fillStyle=d.col;ctx.beginPath();ctx.arc(c.x,c.y,12,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle='rgba(255,255,255,.45)';ctx.lineWidth=2;ctx.stroke();
  ctx.shadowBlur=0;
  ctx.fillStyle='rgba(255,255,255,.45)';ctx.beginPath();ctx.ellipse(c.x-3,c.y-3,4,2.5,-.4,0,Math.PI*2);ctx.fill();
  ctx.fillStyle='rgba(0,0,0,.6)';ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
  ctx.fillText(d.glyph,c.x,c.y+.5);
  ctx.restore();
}
...
function drawPlayer(){
  ...
  if(!crouching){
    const lA=leg*.36;
    ctx.save();ctx.translate(-8,0);ctx.rotate(-lA);
    ctx.beginPath();ctx.rect(-5.5,-26,11,26);fill('#1565C0');ctx.restore();
    ctx.save();ctx.translate(8,0);ctx.rotate(lA);
    ctx.beginPath();ctx.rect(-5.5,-26,11,26);fill('#1565C0');ctx.restore();
    ctx.save();ctx.translate(-8+lA*10,0);
    ctx.beginPath();rr(-9,-5,20,9,3);fill('#212121',1);
    ctx.fillStyle='rgba(255,255,255,.5)';ctx.fillRect(-7,-1,13,2);ctx.restore();
    ctx.save();ctx.translate(8-lA*10,0);
    ctx.beginPath();rr(-11,-5,20,9,3);fill('#212121',1);
    ctx.fillStyle='rgba(255,255,255,.5)';ctx.fillRect(-9,-1,13,2);ctx.restore();
    ctx.save();ctx.translate(-17,-36);ctx.rotate(lA*.42);
    ctx.beginPath();ctx.rect(-4.5,0,9,20);fill(jc,1.5);ctx.restore();
    ctx.save();ctx.translate(17,-36);ctx.rotate(-lA*.42);
    ctx.beginPath();ctx.rect(-4.5,0,9,20);fill(jc,1.5);ctx.restore();
    ...
    ctx.fillStyle='rgba(255,255,255,.48)';ctx.font='bold 9px sans-serif';
    ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('11',0,-ch*.26);
...
function drawWarnings(){
  obstacles.forEach(o=>{
    if(o.y<BASE_Y-140)return;
    if(o.lane!==-1&&o.lane!==P.lane)return;
    const t=Math.max(0,(o.y-(BASE_Y-140))/140);
    const pulse=.55+.45*Math.abs(Math.sin(frame*.3));
    ctx.save();ctx.globalAlpha=t*pulse;
    if(o.layer==='ground'&&P.jumpH<JMP_CLR){
      ctx.shadowColor='#FF1744';ctx.shadowBlur=20;ctx.fillStyle='#FF1744';
      const ay=P.y+26;
      ctx.beginPath();ctx.moveTo(P.x,ay-22);ctx.lineTo(P.x-16,ay);ctx.lineTo(P.x+16,ay);ctx.closePath();ctx.fill();
    } else if(o.layer==='air'&&!P.crouching&&P.jumpH<80){
      ctx.shadowColor='#00E5FF';ctx.shadowBlur=20;ctx.fillStyle='#00E5FF';
      const ay=P.y-P.h-48;
      ctx.beginPath();ctx.moveTo(P.x,ay+22);ctx.lineTo(P.x-16,ay);ctx.lineTo(P.x+16,ay);ctx.closePath();ctx.fill();
    }
...
function drawLaneIndicator(){
  const lx=LANE_X[P.lane],iy=BASE_Y+20;
  const a=.4+Math.sin(frame*.22)*.2;
  ctx.save();ctx.globalAlpha=a;ctx.shadowColor='#E040FB';ctx.shadowBlur=14;
  ctx.fillStyle='#E040FB';ctx.beginPath();
  ctx.moveTo(lx,iy);ctx.lineTo(lx-12,iy+16);ctx.lineTo(lx+12,iy+16);ctx.closePath();ctx.fill();
  ctx.shadowBlur=0;ctx.restore();
}
...
function drawStart(){
  ...
  guide.forEach((g,i)=>{
    ...
    ctx.fillStyle='rgba(255,255,255,.85)';ctx.font='13px sans-serif';ctx.textAlign='left';
    ctx.fillText(`${g.icon}  ${g.tip}`,38,gy+2);
    ctx.fillStyle=g.bc;ctx.font='bold 13px sans-serif';ctx.textAlign='right';
    ctx.fillText(g.hint,W-36,gy+2);
  });
  ctx.textAlign='center';
  ctx.fillStyle='rgba(180,160,255,.45)';ctx.font='11px sans-serif';
  ctx.fillText('← → faixa  ·  ↑/Espaço pular  ·  ↓ agachar',W/2,384);
  ...
  ctx.fillStyle='#fff';ctx.font='bold 19px sans-serif';ctx.shadowColor='rgba(0,0,0,.6)';ctx.shadowBlur=4;
...
function drawDead(){
  ...
  pb.addColorStop(0,'#ef5350');pb.addColorStop(1,'#7c4dff');
  ctx.strokeStyle=pb;ctx.lineWidth=2.5;ctx.stroke();
  ...
  if(score>0&&Math.floor(score)>=best){
    ctx.fillStyle='#00E5FF';ctx.font='bold 14px sans-serif';ctx.fillText('★ Novo recorde! ★',W/2,H/2+6);
  }
...
function loop(){
  ...
  for(let i=obstacles.length-1;i>=0;i--){
    ...
    if(o.y>H+30){obstacles.splice(i,1);continue;}
    if(kills(o)){died=true;break;}
  }
  for(let i=cards.length-1;i>=0;i--){
    const c=cards[i];c.y+=spd;
    if(P.magnet>0){const dx=P.x-c.x,dy=(P.y-P.jumpH)-c.y,d=Math.sqrt(dx*dx+dy*dy);if(d<130&&d>1){c.x+=dx/d*5;c.y+=dy/d*5;}}
    if(c.y>H+20){cards.splice(i,1);continue;}
    const pY=P.y-P.jumpH,pH=P.crouching?P.h*.48:P.h*.65;
    if(Math.abs(P.x-c.x)<(P.w+c.w)*.5&&Math.abs(pY-c.y)<(pH+c.h)*.5){collectCard(c);cards.splice(i,1);}
  }
  for(let i=parts.length-1;i>=0;i--){
    const p=parts[i];p.x+=p.vx;p.y+=p.vy;p.vy+=.16;p.life-=p.dec;
    if(p.life<=0)parts.splice(i,1);
  }
  for(let i=popups.length-1;i>=0;i--){
    const p=popups[i];p.y-=1.9;p.life-=.025;
